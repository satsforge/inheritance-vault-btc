...
async function main() {
  ...
  const csp = [
    ...
    "img-src 'self' data:",
    "font-src 'self'",
    // Fully air-gapped: address/PSBT construction is pure public-key math, and
    // the UTXO used to build a claim is entered manually (see README) rather
    // than fetched, so unlike the suite's watch-only tools this one needs no
    // network access at all - same category as Paper Wallet BTC.
    "connect-src 'none'",
    "frame-src 'none'",
    "object-src 'none'",
    "base-uri 'none'",
